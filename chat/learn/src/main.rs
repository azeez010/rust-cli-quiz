...
fn main(){
    learn_option_enums();
    println!("{:?}", Test::Truth(10));
    // let practice: [u8; 5] =d [1,2,3,4,5];
    // println!("{:?} {}", practice, practice[4]);
    ...
    // println!("{:?}", practice.into_iter().collect::<Vec<_>>());
    // println!("{:?}", practice.into_iter()
    // .map(| x | 9 * x + 1)
    // .filter(|x| x % 2 == 0)
    // .collect::<Vec<_>>());
    // println!("{:?}", practice.into_iter());    
    
    // let mut be: Vec<String> = practice.into_iter().map(|x| x.to_string()).collect::<Vec<_>>();
    // be.push("hello".to_string());
    // println!("{:?}", be);
    // for i in practice.into_iter() {
    //     println!("{}", i);
    // }
}
